...
class FmsAbordajesScreen extends StatefulWidget {
  ...
  const FmsAbordajesScreen({super.key, this.onToggleSidebar, this.usuario});

  @override
  State<FmsAbordajesScreen> createState() => _FmsAbordajesScreenState();
}

class _FmsAbordajesScreenState extends State<FmsAbordajesScreen> {
  ...
  List<Map<String, dynamic>> _obtenerEstadisticasAgrupadas(String tipo) {
    ...
    for (var row in _abordajesFiltrados) {
      String clave = '';
      if (tipo == 'opm') {
        clave = (row['nombre']?.toString() ?? row['operador']?.toString() ?? 'Sin Operador').trim().toUpperCase();
      } else {
        clave = (row['supervisor']?.toString() ?? 'Sin Supervisor').trim().toUpperCase();
      }
      if (clave.isEmpty || clave == 'NULL') clave = 'DESCONOCIDO';
  ...
  Widget _buildTarjetasEstadisticas() {
    int total = _abordajesFiltrados.length;
    int realizados = _abordajesFiltrados.where((r) => _determinarEstado(r) == 'REALIZADO').length;
    int pendientes = total - realizados;
    double avance = total == 0 ? 0 : (realizados / total) * 100;
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Wrap(
        ...
        children: [
          ...
          _buildInfoCard('Pendientes', pendientes.toString(), Icons.pending_actions_rounded, Colors.orange, cardWidth),
          _buildInfoCard('Avance General', '${avance.toStringAsFixed(1)}%', Icons.pie_chart_rounded, Colors.purple, cardWidth),
        ],
      );
    });
  }

  Widget _buildInfoCard(String titulo, String valor, IconData icono, MaterialColor color, double width) {
    return Container(
      width: width,
      ...
      child: Column(
        ...
        children: [
          ...
          Text(valor, style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w900, color: Colors.black87)),
        ],
      ),
    );
  }
  ...
  Widget _buildTablasResumen() {
    var statsSup = _obtenerEstadisticasAgrupadas('supervisor');
    var statsOpm = _obtenerEstadisticasAgrupadas('opm');

    Widget supWidget = _buildTablaDesempeno('Eventos por Supervisor', 'SUPERVISOR', statsSup);
    Widget opmWidget = _buildTablaDesempeno('Eventos por Operador', 'OPERADOR', statsOpm);
  ...
  void _abrirModalGestionarAbordaje(Map<String, dynamic> row) {
    ...
    Future<void> capturarFotoAbordaje(StateSetter setModalState) async {
      try {
        ...
        if (pickedFile != null) {
          setModalState(() {
            fotoEvidenciaFile = File(pickedFile.path);
                ...
                child: SingleChildScrollView(
                  child: Column(
                    ...
                    children: [
                      ...
                      if (isMobileModal) ...[
                        _buildCajaFotoAbordaje(fotoEvidenciaFile, fotoEvidenciaNombre, row['foto_abordaje'], () => capturarFotoAbordaje(setModalState), setModalState, esSoloLectura, tieneFotoPrevia),
                        const SizedBox(height: 16),
                                  ...
                                  if (puntosFirma.isNotEmpty) {
                                    try {
                                      ...
                                      if (byteData != null) {
                                        ...
                                        urlFirma = await ApiService.subirFoto('firma_opm', firmaFile);
                                      }
                                    } catch (e) {
