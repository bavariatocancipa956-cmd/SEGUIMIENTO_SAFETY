...
class FmsTendenciasScreen extends StatefulWidget {
  final VoidCallback? onToggleSidebar;

  const FmsTendenciasScreen({super.key, this.onToggleSidebar});

  @override
  State<FmsTendenciasScreen> createState() => _FmsTendenciasScreenState();
}

class _FmsTendenciasScreenState extends State<FmsTendenciasScreen> {
  // ---------------------------------------------------------------------------
  // 📅 FILTROS DE TENDENCIA
  // ---------------------------------------------------------------------------
  int _anoSeleccionado = DateTime.now().year;
  String _eventoSeleccionado = 'TODOS';
  String _filtroTextoSupervisor = '';

  // ---------------------------------------------------------------------------
  // 🔢 ESTADO DE ORDENAMIENTO (Por defecto: TOTAL de Mayor a Menor)
  // -1: Supervisor | 0..11: Enero..Diciembre | 12: Total General
  // ---------------------------------------------------------------------------
  ...
  void _mostrarModalDetalleEventos({
    ...
    final eventosDetalle = _reportesFms.where((item) {
      ...
      if (supervisor != 'TOTAL GENERAL') {
        String sup = (item['supervisor'] ?? '').toString().trim().toUpperCase();
        if (sup != supervisor.trim().toUpperCase()) return false;
      }
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Row(
                            children: [
                              Container(
                                ...
                                child: const Icon(Icons.list_alt_rounded, color: Color(0xFFDC2626), size: 22),
                              ),
                          ...
                          Text(
                            '$supervisor • $periodoTexto • $_anoSeleccionado (${eventosDetalle.length} Registros)',
                          ...
                          child: DataTable(
                            ...
                            rows: eventosDetalle.map((e) {
                              return DataRow(cells: [
                                ...
                                DataCell(Text(e['nombre']?.toString() ?? '-', style: const TextStyle(fontWeight: FontWeight.bold))),
                                DataCell(
                                  Container(
  ...
  // ---------------------------------------------------------------------------
  // 📊 MOTOR DE MATRIZ DE TENDENCIAS Y ORDENAMIENTO
  // ---------------------------------------------------------------------------
  Map<String, dynamic> _procesarMatrizTendencias() {
    ...
    Set<String> supervisoresSet = {};
    for (var r in _reportesFms) {
      String sup = (r['supervisor'] ?? '').toString().trim();
      if (sup.isNotEmpty) supervisoresSet.add(sup);
    }
    List<String> todosSupervisores = supervisoresSet.toList()..sort();

    Map<String, List<int>> matriz = {};
    Map<String, int> totalesSupervisor = {};

    for (var sup in todosSupervisores) {
      matriz[sup] = List<int>.filled(12, 0);
      totalesSupervisor[sup] = 0;
    }

    for (var r in reportesFiltrados) {
      String sup = (r['supervisor'] ?? '').toString().trim();
      if (matriz.containsKey(sup)) {
        DateTime dt = r['fecha_dt'] as DateTime;
        int mesIdx = dt.month - 1;
        matriz[sup]![mesIdx]++;
        totalesSupervisor[sup] = (totalesSupervisor[sup] ?? 0) + 1;
      }
    }

    // Filtrar supervisores con Total > 0
    List<String> supervisoresActivos = todosSupervisores.where((sup) {
      bool tieneEventos = (totalesSupervisor[sup] ?? 0) > 0;
      bool coincideBusqueda = _filtroTextoSupervisor.isEmpty ||
          sup.toLowerCase().contains(_filtroTextoSupervisor.toLowerCase().trim());
      return tieneEventos && coincideBusqueda;
    }).toList();

    // Ordenamiento dinámico
    supervisoresActivos.sort((a, b) {
      if (_columnaOrden == 12) {
        int valA = totalesSupervisor[a] ?? 0;
        int valB = totalesSupervisor[b] ?? 0;
        return _ordenAscendente ? valA.compareTo(valB) : valB.compareTo(valA);
      } else if (_columnaOrden >= 0 && _columnaOrden < 12) {
        int valA = matriz[a]?[_columnaOrden] ?? 0;
        int valB = matriz[b]?[_columnaOrden] ?? 0;
        return _ordenAscendente ? valA.compareTo(valB) : valB.compareTo(valA);
      } else {
    ...
    List<int> totalesMeses = List<int>.filled(12, 0);
    int granTotal = 0;

    for (int m = 0; m < 12; m++) {
      for (var sup in supervisoresActivos) {
        totalesMeses[m] += matriz[sup]![m];
      }
      granTotal += totalesMeses[m];
    }

    int maxMesValor = 0;
    String mesPico = 'N/A';
    for (int m = 0; m < 12; m++) {
      if (totalesMeses[m] > maxMesValor) {
        maxMesValor = totalesMeses[m];
        mesPico = _meses[m];
      }
    }

    return {
      'supervisores': supervisoresActivos,
      'matriz': matriz,
      'totalesSupervisor': totalesSupervisor,
      'totalesMeses': totalesMeses,
      'granTotal': granTotal,
      'mesPico': mesPico,
      'maxMesValor': maxMesValor,
    };
  }
  ...
  Widget build(BuildContext context) {
    ...
    final dataProcesada = _procesarMatrizTendencias();
    final List<String> supervisores = dataProcesada['supervisores'];
    final Map<String, List<int>> matriz = dataProcesada['matriz'];
    final Map<String, int> totalesSupervisor = dataProcesada['totalesSupervisor'];
    final List<int> totalesMeses = dataProcesada['totalesMeses'];
    final int granTotal = dataProcesada['granTotal'];
    final String mesPico = dataProcesada['mesPico'];
    final int maxMesValor = dataProcesada['maxMesValor'];
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildKpisResumen(granTotal, supervisores.length, mesPico, maxMesValor),
            const SizedBox(height: 20),
            _buildBarraConvenciones(supervisores.length),
            ...
            if (supervisores.isEmpty)
              Container(
                ...
                child: const Center(
                  child: Text(
                    '🛡️ No hay supervisores con incidentes registrados para el año y filtros seleccionados.',
                    style: TextStyle(fontSize: 15, color: Color(0xFF059669), fontWeight: FontWeight.bold),
                  ),
                ),
              )
            else
              _buildTablaTendencias(supervisores, matriz, totalesSupervisor, totalesMeses, granTotal),
          ],
        ),
      ),
    );
        ...
        children: [
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<int>(
                    ...
                    onChanged: (val) {
                      if (val != null) setState(() => _anoSeleccionado = val);
                    },
                  ),
                ),
              ),
              ...
              Container(
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    onChanged: (val) {
                      if (val != null) setState(() => _eventoSeleccionado = val);
                    },
                  ),
                ),
              ),
          ...
          SizedBox(
            ...
            child: Column(
              ...
              mainAxisSize: MainAxisSize.min,
              children: [
                const Text('BUSCAR SUPERVISOR', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w800, color: Color(0xFF64748B), letterSpacing: 0.5)),
                const SizedBox(height: 6),
                SizedBox(
                  height: 44,
                  child: TextField(
                    onChanged: (val) => setState(() => _filtroTextoSupervisor = val),
  ...
  // ---------------------------------------------------------------------------
  Widget _buildKpisResumen(int totalAno, int supsActivos, String mesPico, int maxMesValor) {
    return LayoutBuilder(builder: (context, constraints) {
      bool isWide = constraints.maxWidth > 900;
      List<Widget> cards = [
        _buildKpiCard('TOTAL INCIDENTES EN EL AÑO', '$totalAno', const Color(0xFFDC2626), Icons.warning_amber_rounded),
        _buildKpiCard('SUPERVISORES CON NOVEDAD', '$supsActivos', const Color(0xFF2563EB), Icons.badge_outlined),
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Text(
                'Clic en cualquier número para ver detalle • $totalMostrados supervisores',
  ...
  Widget _buildTablaTendencias(
    ...
    const double colSupervisorWidth = 240;
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  children: [
                    _buildCeldaCabecera('SUPERVISOR', colSupervisorWidth, -1, textAlign: TextAlign.left),
              ...
              ...supervisores.asMap().entries.map((entry) {
                int index = entry.key;
                String sup = entry.value;
                List<int> valoresMeses = matriz[sup] ?? List<int>.filled(12, 0);
                int totalSup = totalesSupervisor[sup] ?? 0;
                        ...
                        child: Row(
                          children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: index < 3 ? const Color(0xFFFEE2E2) : const Color(0xFFF1F5F9),
                                borderRadius: BorderRadius.circular(6),
                              ),
                              child: Text(
                                '${index + 1}',
                                style: TextStyle(
                                  ...
                                  color: index < 3 ? const Color(0xFFDC2626) : const Color(0xFF64748B),
                                ),
                              ),
                            ),
                        ...
                        child: totalSup > 0
                          ...
                          child: InkWell(
                            ...
                            child: Padding(
                              ...
                              child: Tooltip(
                                message: 'Ver eventos totales del supervisor',
