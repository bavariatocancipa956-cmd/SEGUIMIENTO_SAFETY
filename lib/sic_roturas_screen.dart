...
class _SicRoturasScreenState extends State<SicRoturasScreen> {
    ...
    while (mounted) {
      if (controller.hasClients) {
        await controller.animateTo(
          controller.offset + 10000,
          duration: Duration(seconds: (10000 / velocidad).round()),
          curve: Curves.linear,
        );
      } else {
      ...
      if (response.statusCode == 200) {
        ...
        for (var fila in datosRaw) {
          if (fila is Map) {
            ...
            // Carga libre: Se traen todos los datos para llenar los filtros correctamente
            ...
            String tRaw = (mapa['tipo_material']?.toString() ?? '').trim().toUpperCase();
            String tipoLimpio = (tRaw == 'NULL' || tRaw == '[NULL]' || tRaw.isEmpty) ? 'SIN DEFINIR' : tRaw;
            mapa['tipo_material_limpio'] = tipoLimpio;

            if (t.isNotEmpty) turnosSet.add(t);
            if (s.isNotEmpty) supervisoresSet.add(s);
            if (z.isNotEmpty) zonasSet.add(z);
            if (wqi != 'NO ASIGNADO') wqiSet.add(wqi);
    ...
    for (var r in _todosLosReportes) {
      ...
      if (rawFecha != null && rawFecha.isNotEmpty) {
        ...
        if (dt != null && dt.year == _fechaSeleccionada.year && dt.month == _fechaSeleccionada.month && dt.day == _fechaSeleccionada.day) {
          ...
          String tipoMat = r['tipo_material_limpio']?.toString() ?? '';
          ...
          if ((_wqiSeleccionado == 'SI' || _wqiSeleccionado == 'SÍ') && tipoMat != 'PT') {
            continue;
          }

          // ---------------------------------------------------------------------------
          // LÓGICA CORREGIDA: TOMAR LA HORA DESDE LA COLUMNA "FRANJA"
          // ---------------------------------------------------------------------------
          int hora = dt.hour; // Valor por defecto por si la franja está vacía
          String franjaRaw = r['franja']?.toString().trim() ?? '';

          if (franjaRaw.isNotEmpty && franjaRaw.toUpperCase() != 'NULL' && franjaRaw.contains(':')) {
            int? parsedHour = int.tryParse(franjaRaw.split(':')[0]); // Extrae "13" de "13:00"
            if (parsedHour != null) {
              hora = parsedHour;
            }
          }
          ...
          double hlTotalReporte = 0.0;
          String hlStr = (r['hl_total']?.toString() ?? r['hl']?.toString() ?? '0').replaceAll(',', '.');
          if (hlStr.toUpperCase() != 'NULL' && hlStr.isNotEmpty) {
            hlTotalReporte = double.tryParse(hlStr) ?? 0.0;
          }

          String operador = r['personal']?.toString().trim() ?? 'NO REGISTRADO';
          if (operador.isEmpty || operador.toUpperCase() == 'NULL') operador = 'NO REGISTRADO';

          _roturasPorHora[hora] = (_roturasPorHora[hora] ?? 0.0) + cant;
          _roturasPorOperador[operador] = (_roturasPorOperador[operador] ?? 0) + cant;
          _totalHlDia += hlTotalReporte;

          String causal = r['causal']?.toString().trim() ?? 'Desconocida';
          if (causal.toUpperCase() == 'NULL') causal = 'Desconocida';
          String plan = r['plan_reaccion']?.toString().trim() ?? '';
          if (plan.toUpperCase() == 'NULL') plan = '';
  ...
  Widget build(BuildContext context) {
    ...
    String tituloRoturas = (_wqiSeleccionado == 'SI' || _wqiSeleccionado == 'SÍ') ? 'TOTAL ROTURAS (PT)' : 'TOTAL ROTURAS';
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
                ...
                Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 12),
                      Expanded(child: _buildFilterColumn('Zona', _buildDropdown(_zonaSeleccionada, _listaZonas, (v) => setState(() { _zonaSeleccionada = v!; _procesarDatosDelDia(); })))),
                      const SizedBox(width: 12),
                      Expanded(child: _buildFilterColumn('Turno', _buildDropdown(_turnoSeleccionado, _listaTurnos, (v) => setState(() { _turnoSeleccionado = v!; _procesarDatosDelDia(); })))),
                      const SizedBox(width: 12),
                      Expanded(child: _buildFilterColumn('WQI', _buildDropdown(_wqiSeleccionado, _listaWqi, (v) => setState(() { _wqiSeleccionado = v!; _procesarDatosDelDia(); })))),
                      const SizedBox(width: 12),
                      Expanded(flex: 2, child: _buildFilterColumn('Supervisor', _buildDropdown(_supervisorSeleccionado, _listaSupervisores, (v) => setState(() { _supervisorSeleccionado = v!; _procesarDatosDelDia(); })))),
                    ],
                  ),
                ),
              ...
              Row(
                children: [
                  Expanded(child: _buildKPICard(tituloRoturas, '${totalRoturasDia.toInt()}', 'Unidades físicas reportadas', const Color(0xFF3B82F6))),
                  const SizedBox(width: 16),
                  Expanded(child: _buildKPICard('TOTAL VOLUMEN', '${_totalHlDia.toStringAsFixed(2)} HL', 'Hectolitros totales', const Color(0xFF06B6D4))),
                  const SizedBox(width: 16),
                      ...
                      child: Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Expanded(
                              child: CustomPaint(
                                painter: _AreaChartPainter(datos: _roturasPorHora, meta: _metaPorHora, isFullScreen: widget.isFullScreen),
                                child: Container(),
                              ),
                            ),
                          ],
                                ...
                                children: [
                                  ...
                                  Container(
                                    ...
                                    child: Row(
                                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                      children: [
                                        const Text('PERSONAL ASIGNADO', style: TextStyle(color: Color(0xFF64748B), fontSize: 10, fontWeight: FontWeight.w800)),
                                        Text((_wqiSeleccionado == 'SI' || _wqiSeleccionado == 'SÍ') ? 'ROTURAS (PT)' : 'ROTURAS', style: const TextStyle(color: Color(0xFF64748B), fontSize: 10, fontWeight: FontWeight.w800)),
                                      ],
                                    ),
                                  ),
                                  Expanded(
                                    child: operadoresOrdenados.isEmpty
                                        ? const Center(child: Text('Sin datos que cumplan el filtro', style: TextStyle(color: Colors.grey, fontSize: 12)))
                                        : ListView.builder(
                                      controller: _scrollOperadores,
                                      padding: EdgeInsets.zero,
                                      itemBuilder: (context, index) {
                                        var op = operadoresOrdenados[index % operadoresOrdenados.length];
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: _eventosFiltrados.isEmpty
                                        ? const Center(child: Text('Sin eventos que cumplan el filtro', style: TextStyle(color: Colors.grey, fontSize: 12)))
                                        : ListView.builder(
                                      controller: _scrollEventos,
                                      padding: EdgeInsets.zero,
                                      itemBuilder: (context, index) {
                                        var e = _eventosFiltrados[index % _eventosFiltrados.length];
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<String>(
          ...
          items: items.map((e) => DropdownMenuItem(value: e, child: Text(e, overflow: TextOverflow.ellipsis, maxLines: 1))).toList(),
          onChanged: onChanged,
        ),
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(mainValue, style: TextStyle(fontSize: 34, fontWeight: FontWeight.w900, color: color, height: 1)),
...
class _AreaChartPainter extends CustomPainter {
  ...
  _AreaChartPainter({required this.datos, required this.meta, this.isFullScreen = false});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int v in yValues) {
      ...
      TextPainter tpY = TextPainter(
        text: TextSpan(text: v.toString(), style: TextStyle(color: v == meta.toInt() ? const Color(0xFF10B981) : const Color(0xFF64748B), fontSize: 11, fontWeight: FontWeight.bold)),
    ...
    for (int i = 0; i < 24; i++) {
      ...
      TextPainter tpX = TextPainter(
        text: TextSpan(text: horaText, style: TextStyle(color: const Color(0xFF475569), fontSize: isFullScreen ? 14 : 11, fontWeight: FontWeight.bold)),
        textDirection: TextDirection.ltr,
      )..layout();

      canvas.save();
      canvas.translate(x, pTop + drawHeight + 15);
      canvas.rotate(-0.5);
    ...
    if (puntos.isNotEmpty) {
      ...
      for (int i = 0; i < puntos.length; i++) {
        ...
        TextPainter tpVal = TextPainter(
          text: TextSpan(text: val.toInt().toString(), style: const TextStyle(color: Color(0xFF0F172A), fontSize: 12, fontWeight: FontWeight.w900)),
          textDirection: TextDirection.ltr,
        )..layout();
